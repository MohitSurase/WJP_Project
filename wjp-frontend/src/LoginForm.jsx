...
function LoginForm() {
  ...
  let loginAction = async (e) => {
    ...
    try {
      ...
      let url = "http://localhost:8080/loginStudent";
      let params = { email: user.email, password: user.password };

      let response = await axios.get(url, { params });
      let responseData = response.data;

      if (responseData === "User Login Successful") {
        // Login successful
        localStorage.setItem("loginStatus", "true");
        localStorage.setItem("userRole", "student");
        localStorage.setItem("email", user.email);
        window.open("/functionalpage"); // Open functional page in the same tab/window
      } else {
        // Invalid credentials
  ...
  return (
    <>
      ...
      <div className="col-lg-5 m-auto d-block ">
        ...
        >
          <h1 className="mb-5 mt-5 fs-1 mx-auto ">Student Login</h1>
        </div>
